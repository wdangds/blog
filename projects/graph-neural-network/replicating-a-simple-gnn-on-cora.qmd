...
[Logistic regression](../../teaching/cds-303/chapter-4.qmd#logistic-regression) or a [decision tree](../../teaching/cds-303/chapter-5.qmd) uses **only the row's features**. But on graphs, who we're connected to matters (homophilly: connected nodes often share labels). A GNN layer explicitly *aggregates neighbor information* before classifying - so it's like "logistic regression on denoised/neighbor-aware features".
...
![base-train-val-loss.png](../../assets/images/projects/graph-neural-network/base-train-val-loss.png)
**What we see**:
