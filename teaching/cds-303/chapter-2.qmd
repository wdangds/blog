...
Suppose a company uses a complex [neural network](chapter-8.qmd) to predict chemical yield with excellent accuracy. If the model cannot explain which process variables influence yield, engineers may be unable to improve the process. In that case, predictive performance does not translate into business value.
