...
::: {.callout-tip title="Example: Missing Sunday Sales"}
...
```{python}
#| label: fig-missing-sunday-sales
#| fig-cap: "Imputing a Missing Sunday Sales Value"
import numpy as np
import pandas as pd
...
#| fig-cap: "Comparing raw, min-max, max-abs, and standardized feature scales"
import numpy as np
import pandas as pd
import matplotlib.pyplot as plt
...
#| fig-cap: "One-hot encoding representation of movie genres"
import pandas as pd
import matplotlib.pyplot as plt
...
```{python}
#| label: fig-email-word-frequencies
#| fig-cap: "Word frequencies in the example email corpus"
import pandas as pd
import matplotlib.pyplot as plt
...
::: {.callout-note title="Definition: Data Augmentation"}
...
#| fig-cap: "Simple image augmentation examples"
import numpy as np
import matplotlib.pyplot as plt
...
::: {.callout-note title="Definition: Machine Learning"}
...
This is why model design is iterative, as discussed [previously](chapter-1.qmd#the-data-science-cycle). Early results often reveal that the target is poorly defined, a feature leaks future information, a model is too complex, or the metric does not match the real decision problem.
...
#| fig-cap: "Polynomial models with different complexity on the same dataset"
import numpy as np
import matplotlib.pyplot as plt
...
- Number of trees in a [random forest](chapter-5.qmd#bagging-and-random-forests)
- Learning rate in gradient boosting
- Regularization strength in [logistic regression](chapter-4.qmd#logistic-regression)
- Number of layers in a [neural network](chapter-8.qmd)
...
### 11.3. [Bayesian Optimization](chapter-9.qmd)

[Bayesian optimization](chapter-9.qmd)[^3] treats hyperparameter tuning as a sequential decision problem. Instead of blindly trying combinations, it builds a probabilistic model of performance and chooses promising configurations to evaluate next. 

```{python}
#| label: fig-hyperparameter-surface
#| fig-cap: "Illustrative hyperparameter performance surface"
import numpy as np
import matplotlib.pyplot as plt
...
## 13. [Bayesian](chapter-9.qmd) and Optimization-Driven model Design

In [Bayesian](chapter-9.qmd) model design, the analyst specifies prior beliefs, a likelihood model, and a procedure for updating beliefs after observing data. This approach is useful when uncertainty matters, sample sizes are limited, or prior scientific knowledge should be incorporated explicitly. 
...
```{python}
#| label: fig-gaussian-process-uncertainty
#| fig-cap: "Gaussian process regression with uncertainty bands"
import numpy as np
import matplotlib.pyplot as plt
...
Neural Architecture Search, or NAS, automates the search for [neural network](chapter-8.qmd) architectures. Instead of manually designing every layer, filter size, and connection pattern, NAS uses search algorithms to explore possible architectures.
...
Next chapter: [Data Modeling - Regression](chapter-4.qmd)
