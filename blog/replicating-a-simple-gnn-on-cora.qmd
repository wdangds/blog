...
tags:
  ...
  - neural-network
date: 2025-10-27
description: A hands-on replication of a graph convolutional network for node classification on the Cora citation network.
---
...
[Logistic regression](../teaching/cds-303/chapter-4.qmd#logistic-regression) or a [decision tree](../teaching/cds-303/chapter-5.qmd) uses **only the row's features**. But on graphs, who we're connected to matters (homophily: connected nodes often share labels). A GNN layer explicitly *aggregates neighbor information* before classifying - so it's like "logistic regression on denoised/neighbor-aware features".
...
::: {.callout-tip title="Intuition vs. logistic regression"}
...
::: {.callout-note title="Why self-loops and normalization?"}
...
![Training and validation loss for the baseline GCN](../assets/images/blog/base-train-val-loss.png)
**What we see**:
...
We will train [logistic regression](../teaching/cds-303/chapter-4.qmd#logistic-regression) and a small **MLP** on `X` alone (ignore edges). Compare their test accuracy/macro-f1 to the GCN.
