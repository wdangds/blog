...
The core idea of MCMC is to construct an **[[Irreducible and aperiodic Markov chains|irreducible and aperiodic]] Markov chain $(X_0, X_1,...)$** whose *unique stationary distribution is the desired probability distribution $\pi$.*

The [[Stationary distributions#3. Convergence to the Stationary Distribution|Markov chain convergence theorem]] guarantees that the distribution of the chain at time $n$ converges to $\pi$ as $n\longrightarrow \infty$.

By running the chain for a sufficiently long time $n$, the distribution of $X_n$ will be very close to $\pi$. This provides an approximation that can be made arbitrarily good by increasing the running time.

A key insight is that **constructing such a Markov chain can be easier than directly constructing a random variable with the desired distribution**.